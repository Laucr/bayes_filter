...
class TextFilter(object):
	...
	# get irrelevant words list
	@staticmethod
	def _get_irrelevant_wd_list():
		if os.path.exists('irrelevant.txt'):
			with open('irrelevant.txt') as f:
				text = f.read()
...
# modify with db
		...
		with open(filename) as f:
			...
			for word in text.split():
				if word not in tmp:
					tmp[word] = 0
				tmp[word] += 1
		# remove irrelevant here
		for wd in TextFilter._get_irrelevant_wd_list():
			if wd in tmp:
				del tmp[wd]
		return tmp
	...
	def report(self):
		with codecs.open('report.log', 'w', 'utf-8') as f:
			ops = self.op_detect(self.get_sets(self._te_set_dir))
			f.write(ops)
			f.write('\n')
			for key in self.rpt:
	...
	def executor(self):
		self._load_config()
		self._set_attr(self.get_sets(self._tr_set_dir), 'train')
		self._set_attr(self.get_sets(self._te_set_dir), 'test')
		self._set_attr(self._bayes_train(self._tr_set_files), 'tr_wd_list')
		self._set_attr(self._get_prior_prob(), 'prob')
		self._set_attr(self.bayes_filter(self._prior_prob, self._tr_wd_list, self._ful_wd_list, self._te_set_files), 'rpt')
		self.report()
